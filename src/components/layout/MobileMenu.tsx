...
export default function MobileMenu() {
  ...
  return (
    <>
      ...
      >
        <span className={`block w-6 h-0.5 bg-white transition-transform duration-300 origin-center ${open ? "translate-y-2 rotate-45" : ""}`} />
      ...
      <div className="delfin-nav-overlay" data-open={open || undefined} aria-hidden={!open}>
        ...
        <nav className="delfin-nav-panel" aria-label="Mobilna navigacija">
          <div className="flex flex-col h-full px-6 pt-28 pb-10 min-h-screen">

            {/* Nav stavke */}
            <ul className="flex flex-col gap-1 flex-1" role="list">
              {navLinks.map((link, idx) => {
                const delay = `${220 + idx * 80}ms`;
                return (
            ...
            {/* CTA */}
            <div className="delfin-nav-footer flex flex-col gap-3 pt-6 border-t border-white/10">
              <a
                href={WHATSAPP_URL}
                target="_blank"
